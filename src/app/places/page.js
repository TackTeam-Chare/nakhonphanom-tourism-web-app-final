          ...
          {currentPlaces.map((place, index) => (
              <Link className="hover:no-underline" href={`/places/nearby/${place.id}`} key={index}>
                <div className="bg-white rounded-lg shadow-lg overflow-hidden transform hover:scale-105 transition duration-500 ease-in-out">
                  <Image  src={place.image_path} alt={place.name} width={500} height={300} priority className="w-full h-48 object-cover"/>
                  <div className="p-6">
                    <h2 className="text-2xl font-bold mb-2">{place.name}</h2>
                    <p className="text-gray-700 mb-2"><strong>Location:</strong> {place.location}</p>
                    <p className="text-gray-700 mb-2"><strong>Description:</strong> {place.description}</p>
                    <p className="text-gray-700 mb-2"><strong>Type:</strong> {place.category_name}</p>
                    <p className="text-gray-700"><strong>District:</strong> {place.district_name}</p>
                  </div>
                </div>
            </Link>
          ))}
